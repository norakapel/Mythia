...
public class FinalStageMulti implements ActionListener{
	...
	JLabel draw = new JLabel ("It's a draw. Player1 Choose:");
	...
	public void StageThreeMulti(int k, int l, int points1, int points2){
		...
		p3.setSize(300,500);
		p3.setOpaque(false);
		p3.setBounds(w /2-180, h/3 + 50, 360, 500);
		p3.setVisible(true);
	...
	public void Game(){
		if (po1 == po2) {
			...
			draw.setBounds(w/2-200, h/3 - h/20, 400 , 30);
			draw.setFont(new Font("Courier", Font.BOLD, 17));
			p3.add(draw);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getActionCommand().equals("Heads")){
			...
			new java.util.Timer().schedule(new java.util.TimerTask(){
				...
				MovePressed(1,1);
				for (int j=0; j < 3; j++){
					PlayerButtons[1][j].setEnabled(true);
					PlayerButtons[1][j].setEnabled(false);
				}
				new java.util.Timer().schedule(new java.util.TimerTask(){
					@Override
					public void run(){
